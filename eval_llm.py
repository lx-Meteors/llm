...
from models.model_meteor import MeteorConfig, MeteorForCausalLM
...
def init_model(args):
    ...
    if 'model' in args.load_from:
        model = MeteorForCausalLM(MeteorConfig(
    ...
    print(f'Meteor模型参数: {sum(p.numel() for p in model.parameters()) / 1e6:.2f} M(illion)')
    return model.eval().to(args.device), tokenizer

def main():
    parser = argparse.ArgumentParser(description="Meteor模型推理与对话")
